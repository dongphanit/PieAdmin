    ...
    return Row(
      children: [
        ...
        if (!Responsive.isMobile(context))
          Expanded(
            child: Text(
              "${context.watch<MenuControllerA>().screens_title[context.watch<MenuControllerA>().currentSelectedIndex]}",
...
class SearchField extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return TextField(
      onChanged: (value) {
        if (context.read<MenuControllerA>().currentSelectedIndex == 1) {
        } else if (context.read<MenuControllerA>().currentSelectedIndex == 2) {
